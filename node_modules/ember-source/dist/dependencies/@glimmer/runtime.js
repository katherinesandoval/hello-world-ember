import { assign, symbol, Stack, fillNulls, initializeGuid, _WeakSet, dict, emptyArray, EMPTY_STRING_ARRAY, debugToString, decodeHandle, isNonPrimitiveHandle, isErrHandle, unwrapTemplate, unwrapHandle, clearElement, constants, decodeImmediate, encodeHandle, encodeImmediate, isHandle, isSmallInt, buildUntouchableThis } from '@glimmer/util';
...
import { destroy, registerDestructor, associateDestroyableChild, destroyChildren, isDestroyed, isDestroying } from '@glimmer/destroyable';
...
import { CONSTANT_TAG, valueForTag, validateTag, INITIAL, beginTrackFrame, endTrackFrame, consumeTag, track, updateTag, resetTracking, runInTrackingTransaction, createCache, getValue, createUpdatableTag } from '@glimmer/validator';
...
function resolveCurriedValue(curriedValue, args) {
  ...
  while (true) {
    ...
    if (curriedArgs !== null) {
      if (curriedArgs.positional.length > 0) {
        prependArgs = prependArgs === undefined ? curriedArgs.positional : curriedArgs.positional.concat(prependArgs);
      }
...
function invokeModifier(vm, owner, args, definition) {
  let {
    manager
  } = definition;
  let {
  ...
  let state = manager.create(owner, constructing, definition.state, args, dynamicScope, updateOperations);
  let instance = {
    manager,
    state,
    definition
...
, (vm, {
  ...
  if (typeof component === 'string') {
    ...
    definition = constants$$1.component(component);
  }
...
, vm => {
  ...
  if (isCurriedValue(value)) {
    ...
    definition = constants$$1.component(value, true);
...
, (vm, {
  ...
  /* Component */
    ...
    if (isResolved === true) {
      ...
      definition = constants$$1.resolvedComponent(resolvedValue, value);
    } else {
      definition = constants$$1.component(value);
    }
...
var concat = internalHelper(args => {
  let captured = args.positional.capture();
